...
// ฟังก์ชันสำหรับส่ง LINE Message (ใช้ WiFiClientSecure สำหรับ HTTPS)
...
// ฟังก์ชันตรวจสอบและอัปเดตเฟิร์มแวร์อัตโนมัติจากไฟล์ JSON
void checkAndDoOTA() {
  ...
  http.setTimeout(10000); // แก้ปัญหา HTTP Code -11: เพิ่มเวลา Timeout เป็น 10 วินาที
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    if (!error) {
      ...
      if (remoteVersion != CURRENT_VERSION) {
        ...
        // ปิด Connection เดิมเพื่อคืน RAM และ Socket ให้ระบบ
        http.end(); 

        // สร้าง WiFiClient ใหม่แยกต่างหากสำหรับดาวน์โหลดไฟล์ .bin
        ...
        switch (ret) {
          case HTTP_UPDATE_FAILED:
            ...
            // แจ้งเตือน LINE เฉพาะเมื่อล้มเหลว (หลีกเลี่ยงการเรียก SSL ก่อน OTA เพื่อประหยัด RAM)
            sendLineMessage("OTA Update Failed! Error: " + String(httpUpdate.getLastErrorString()));
            break;
